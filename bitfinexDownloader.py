...
class bitfinexDownloader:
    
    def __init__():
        if (os.path.isfile("cache-finex.csv")): #if the file already exists start from the latest date
            self.starttime = datetime.datetime.fromtimestamp(int(str(int(pd.read_csv('cache-finex.csv', header=None).iloc[-1][0]))[:-3])) #read the last timestamp for csv file. Bitstamp takes and returs date date with 3 extra zeros. So that
        else:
    ...
    def loadData():
        ...
        while (start_unixtime < latest_time):
            ...
            df.to_csv('cache-finex.csv',header=None,mode='a') #append the data
    ...
    def saveData():
        '''
        Saves the data by adding header
        '''
        #add the header
        df = pd.read_csv('cache-finex.csv', header=None, index_col=None)
